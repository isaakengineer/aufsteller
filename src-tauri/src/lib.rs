...
fn project_on(name: String) -> WorkspaceIndex {
    ...
    let contents = match fs::read_to_string(&base) {
        ...
        Err(e) => {
            println!(
                "reading workspace index from path = `{}` failed!",
    ...
    for desktop in &index.desktop {
        ...
        new_command
            .arg("-s")
            .arg((workspaces_count - 1).to_string());
...
fn dashboard_config_load(name: String) -> Result<String, String> {
    ...
    match fs::read_to_string(&base) {
        Ok(c) => return Ok(c),
        Err(e) => return Err("reading workspace index from path = `{}` failed!".to_string()),
    }
}
...
fn app_open(exec: String, path: String) {
    ...
    command
        .spawn()
        .expect("failed to open directory with supplied app.");
}
